/*
		...
		Calling the script:
			...
			default: 		nul = [this] execVM "external\LV\heliParadrop.sqf";
			...
			custom: 		nul = [spot, side, allowDamage, captive, distance, direction, flyby, fly height, jump distance,
								...
								group, custom init, ID, MP, doors, classes] execVM "external\LV\heliParadrop.sqf";
...
EXAMPLE: 	nul = [player, 2, false, true, 1000, "random", true, 500, 200, 6, 1, 50, true, false, true, true, player, false, 0.75, nil, nil, 1,false] execVM "external\LV\heliParadrop.sqf";
*/
...
if(_mp)then{if(isNil("LV_GetPlayers"))then{LV_GetPlayers = compileFinal preprocessFile "external\LV\LV_functions\LV_fnc_getPlayers.sqf";};};
...
_spos = [(_landingSpotPos select 0) + (sin _heliDirection) * _heliDistance, (_landingSpotPos select 1) + (cos _heliDirection) * _heliDistance, 250];
_heli = createVehicle [_heliType, _spos, [], 0, "FLY"];
_heli setPosATL [(_landingSpotPos select 0) + (sin _heliDirection) * _heliDistance, (_landingSpotPos select 1) + (cos _heliDirection) * _heliDistance, 250];
...
[_heli,_grp,_flySpot,_landingSpotPos,_heliDistance] spawn {
	...
	//while{([_heli, _flySpot] call BIS_fnc_distance2D)>_heliDistance * .5}do{
	while{(_heli distance2D _flySpot) > (_heliDistance * .5)}do{
		if(!alive _heli || !canMove _heli)exitWith{};
		sleep 5;
	};

	waitUntil{sleep 0.028;(_heli distance2D _landingSpotPos) > (_heliDistance * .8)};//(_heliDistance * .5)};
	//waitUntil{([_heli, _landingSpotPos] call BIS_fnc_distance2D)>_heliDistance * .8};//(_heliDistance * .5)};
	{ deleteVehicle _x; sleep 0.001; } forEach units _grp;
	deleteVehicle _heli;
};

//Wait till it's close enough
//waitUntil{([_heli, _landingSpotPos] call BIS_fnc_distance2D)<_jumpDistanceFromTarget};
waitUntil{sleep 0.026;(_heli distance2D _landingSpotPos) < _jumpDistanceFromTarget};
...
for "_i" from 1 to _jumperAmount step 1 do{
	...
	[_man2,_heli,_openHeight,_smokes,_flares,_chems] spawn{
		...
		waitUntil{sleep 0.013;((getPos _man2)select 2)<_openHeight};
		...
		if(_smokes)then{
			waitUntil{sleep 0.021;((getPos _man2)select 2)<10};
			_smoke = "SmokeShell" createVehicle (getPos _man2);
		};
		if(_flares)then{
			waitUntil{sleep 0.022;((getPos _man2)select 2)<30};
			_flare = "F_40mm_Red" createVehicle [(getPos _man2) select 0,(getPos _man2) select 1,0]; //Chemlight_red
		};
		if(_chems)then{
			waitUntil{sleep 0.019;((getPos _man2)select 2)<30};
	...
	if(typeName _target == "ARRAY")then{ //TARGET is array
		if((!((_target select 0) in allMapMarkers))&&(!((_target select 0) in allGroups)))then{
				...
				if((_target select 0) == "PATROL")then{ ////USE patrol-vD.sqf
					{
						...
						nul = [_x,_cPosition,_cRadius,_doorHandling] execVM "external\LV\patrol-vD.sqf";
						sleep 0.001;
					}forEach units _grp2;
				};
			};
		}else{ //TARGET is array of Markers or Groups
			{
				...
				if(_cycle)then{
					nul = [_x,true] execVM "external\LV\LV_functions\LV_fnc_follow.sqf";
				}else{
					nul = [_x,false] execVM "external\LV\LV_functions\LV_fnc_follow.sqf";
				};
				sleep 2;
			}forEach units _grp2;
		};
		...
		if(_target in allMapMarkers)then{ /////TARGET is single Marker
			{ _x doMove getMarkerPos _target; sleep 0.001; } forEach units _grp2;
		}else{
				{ ////TARGET is single Unit/Object
					_x setVariable ["target0",_target,false];
					_x setVariable ["mDis0", 1000, false];
					nul = [_x] execVM "external\LV\LV_functions\LV_fnc_follow.sqf";
					sleep 0.001;
				}forEach units _grp2;
		};
	};
