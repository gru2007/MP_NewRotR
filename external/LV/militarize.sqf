/*
		...
		default: 	nul = [this] execVM "external\LV\militarize.sqf";

		custom:		nul = [target, side, radius, spawn men, spawn vehicles, still, men ratio, vehicle ratio,
							skills, group, custom init, ID, smokes, doors, classes] execVM "external\LV\militarize.sqf";
...
EXAMPLE: nul = [this,2,50,[true,true],[true,false,true],false,[10,0],0.1,[0.2,0.2,0.2,0.85,0.9,0.75,0.1,0.6,1,1],nil,nil,nil,true,true,["ALL"]] execVM "external\LV\militarize.sqf";
*/
	...
	for "_i" from 1 to _menAmount do{
		...
		if(!_still)then{
			if(_unitType in _menArray)then{
				nul = [_unit,_cPos,_radius,_doorHandling] execVM "external\LV\patrol-vD.sqf";
			}else{
				nul = [_unit,_pos] execVM 'external\LV\patrol-vH.sqf';
			};
		};
...
if((_vehicles select 0)||(_vehicles select 1)||(_vehicles select 2))then{
    for "_i" from 1 to _vehAmount do{
		...
		while{!_validPos}do{
			...
			_range = random _radius;
			_pos0 = [(_centerPos select 0) + (sin _dir) * _range, (_centerPos select 1) + (cos _dir) * _range, 0];
			_pos = [_pos0,0,50,10,_wMode,.25,0] call BIS_fnc_findSafePos;

			if(true)then{ //side > 0
				if(surfaceIsWater _pos)then{
					if(_vehicles select 1)then{
						_driver = [_pos, (_side),_classModuleFilters,nil,nil,false] call LV_fullWaterVehicle;
						if(!_still)then{nul = [vehicle _driver,_pos] execVM 'external\LV\patrol-vH.sqf';};
						_validPos = true;
					}else{
						_wMode = 0;
					};
					...
					if((_vehicles select 0)&&(_vehicles select 2))then{
						...
						if(_whichOne < 3)then{
							...
							if(!_still)then{nul = [_driver,_pos,[200,200]] execVM 'external\LV\patrol-vE.sqf';};
							vehicle _driver flyInHeight 10;
							_validPos = true;
						}else{
							_driver = [_pos, (_side),_classModuleFilters,nil,nil,false] call LV_fullLandVehicle;
							if(!_still)then{nul = [vehicle _driver,_pos] execVM 'external\LV\patrol-vE.sqf';};
							_validPos = true;
						};
					}else{
