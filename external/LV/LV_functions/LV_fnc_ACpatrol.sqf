...
/*
fn_drawLine = compileFinal preprocessFileLineNumbers "fn_drawLine.sqf";
fn_r2Dis = compileFinal preprocessFileLineNumbers "fn_R2Dis.sqf";
fn_r2Ang = compileFinal preprocessFileLineNumbers "fn_R2Ang.sqf";
fn_p2R = compileFinal preprocessFileLineNumbers "fn_P2R.sqf";
		...
		if(unitReady _leader)then{
			...
			while{!_validSpot}do{
				...
				if(!surfaceIsWater (getPos _leader))then{ //land units
					...
					_newPos = [_newPos,0,200,5,2,0,0] call BIS_fnc_findSafePos;
					_validSpot = true;
				};
			...
			if((typeName _pType) == "ARRAY")then{
				...
				{_x doMove _newPos;sleep 0.001;}forEach units _wGroup;
			};
		};
		sleep 1;
		_i = _i + 1;
