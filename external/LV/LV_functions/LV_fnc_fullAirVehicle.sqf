...
_veh = ([_classModuleFilters,[(_side), 3]] call LV_classnames) + ([_classModuleFilters,[(_side), 4]] call LV_classnames);

_veh = [_veh] call LV_validateClassArrays;
if((count _veh) == 0)then{
	_veh = ([[],[(_side), 3]] call LV_classnames) + ([[],[(_side), 4]] call LV_classnames);
};
