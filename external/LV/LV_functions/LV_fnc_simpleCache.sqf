/*ARMA3Alpha function LV_fnc_simpleCache v0.7 - by SPUn / lostvar

		This script caches fillHouse & militarize scripts.

	nul = [[ID's],[players],distance,keep count,MP] execVM "external\LV\LV_functions\LV_fnc_simpleCache.sqf";
	...
	nul = [[13,14],[playerUnit1],500,true,false] execVM "external\LV\LV_functions\LV_fnc_simpleCache.sqf";

*/
if (!isServer)exitWith{};
private ["_tempVehs","_mp","_i","_cPos","_inRange","_aliveCount","_grp","_center","_scriptParams","_id","_ids","_units","_distance","_amountArr","_script","_params","_vehicleAmount","_vehicleArr","_excludeSingleHeliPilot","_inRangeUnits","_pilotCandidate"];
	...
	for[{_i=0}, {_i<(count _ids)}, {_i=_i+1}] do{
		...
		if((isNil("LVgroup"+(str (_ids select _i))+"spawned"))&&(_inRange))then{
			...
			if(_script == "militarize")then{
				call compile format["nul = %1 execVM 'external\LV\militarize.sqf';",_params];
			}else{
				call compile format["nul = %1 execVM 'external\LV\fillHouse.sqf';",_params];
			};
